...
uint64_t rho_model_hash_mix(uint64_t value)
{
...
void rho_model_hash_add(uint64_t& hash, uint64_t value)
{
  hash ^= rho_model_hash_mix(
      value + 0x9e3779b97f4a7c15ULL +
      (hash << 6) + (hash >> 2));
}

void rho_model_hash_add(
    uint64_t& hash, const ShelfSelector& shelf)
{
  rho_model_hash_add(hash, static_cast<uint64_t>(shelf.kind));
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(shelf.value)));
}

void rho_model_hash_add(
    uint64_t& hash, const TransferKey& key)
{
  rho_model_hash_add(hash, key.shelf);
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(key.source)));
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(key.endpoint)));
}

void rho_model_hash_add(
    uint64_t& hash, const TaskId& task)
{
  rho_model_hash_add(hash, task.shelf);
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(task.from)));
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(task.to)));
}

void rho_model_hash_add(
    uint64_t& hash, const RhoColumnKey& column)
{
  rho_model_hash_add(hash, static_cast<uint64_t>(column.kind));
  rho_model_hash_add(hash, column.transfer);
  rho_model_hash_add(hash, column.task);
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(column.task_index)));
  rho_model_hash_add(
      hash, static_cast<uint64_t>(
                static_cast<uint32_t>(column.owner_robot)));
}

uint64_t rho_column_model_hash(
    const RhoColumnModelVersion& model)
{
  uint64_t hash = rho_model_hash_mix(0x52484f4d4f44454cULL);
  rho_model_hash_add(hash, model.ordered_columns.size());
  for (const auto& column : model.ordered_columns)
    rho_model_hash_add(hash, column);
  const auto add_costs = [&](const std::vector<RhoCost>& values) {
    rho_model_hash_add(hash, values.size());
    for (const RhoCost value : values)
      rho_model_hash_add(hash, static_cast<uint64_t>(value));
  };
  add_costs(model.service);
  add_costs(model.urgency);
  add_costs(model.root_delay);
  rho_model_hash_add(
      hash, model.endpoint_conflict_version.size());
  for (const uint64_t value :
       model.endpoint_conflict_version)
    rho_model_hash_add(hash, value);
  rho_model_hash_add(hash, static_cast<uint64_t>(model.mode));
  rho_model_hash_add(hash, model.mode_semantics_version);
  rho_model_hash_add(hash, model.objective_version);
  rho_model_hash_add(hash, model.scaling_version);
  rho_model_hash_add(hash, model.inf_version);
  rho_model_hash_add(hash, model.canonical_version);
  return hash;
}

bool rho_column_model_shape_valid(
    const RhoColumnModelVersion& model)
{
  const size_t columns = model.ordered_columns.size();
  return model.service.size() == columns &&
         model.urgency.size() == columns &&
         model.root_delay.size() == columns &&
         model.endpoint_conflict_version.size() == columns;
}

bool rho_node_state_shape_valid(
    const RhoNodeAssignmentState& state)
{
  const int rows = state.optimum.real_rows;
  const int columns = state.optimum.columns;
  if (rows < 0 || columns < 0 ||
      !rho_column_model_shape_valid(state.column_model) ||
      static_cast<int>(
          state.column_model.ordered_columns.size()) != columns ||
      static_cast<int>(state.row_fingerprint.size()) != rows ||
      static_cast<int>(state.mate.size()) != rows ||
      static_cast<int>(state.anchor_used.size()) != rows)
    return false;
  for (int row = 0; row < rows; ++row) {
    if (static_cast<int>(
            state.row_fingerprint[row].eligibility.size()) != columns ||
        state.row_fingerprint[row].anchor_used !=
            state.anchor_used[row])
      return false;
    if (state.mate[row].has_value() &&
        std::find(
            state.column_model.ordered_columns.begin(),
            state.column_model.ordered_columns.end(),
            *state.mate[row]) ==
            state.column_model.ordered_columns.end())
      return false;
  }
...
{
  model.quick_hash = rho_column_model_hash(model);
}
    ...
    const RhoColumnModelVersion& b)
{
  ...
  return a.ordered_columns == b.ordered_columns &&
         a.service == b.service &&
         a.urgency == b.urgency &&
         a.root_delay == b.root_delay &&
         a.endpoint_conflict_version ==
             b.endpoint_conflict_version &&
         a.mode == b.mode &&
         a.mode_semantics_version == b.mode_semantics_version &&
         a.objective_version == b.objective_version &&
         a.scaling_version == b.scaling_version &&
         a.inf_version == b.inf_version &&
         a.canonical_version == b.canonical_version;
}
...
{
  if (!rho_column_model_shape_valid(parent) ||
      !rho_column_model_shape_valid(current) ||
      parent.ordered_columns.size() !=
          current.ordered_columns.size())
    return RhoIncrementalFallbackReason::SHAPE_CHANGED;
  // The hash is only a fast rejection/acceptance candidate.  Even on an
  // equal hash, exact descriptors remain the authority.
  if (parent.quick_hash == current.quick_hash &&
      rho_column_models_exactly_equal(parent, current))
    return RhoIncrementalFallbackReason::NONE;
  if (parent.mode != current.mode ||
      parent.mode_semantics_version !=
          current.mode_semantics_version)
    return RhoIncrementalFallbackReason::MODE_CHANGED;
  if (parent.objective_version != current.objective_version)
    return RhoIncrementalFallbackReason::
        OBJECTIVE_VERSION_CHANGED;
  if (parent.scaling_version != current.scaling_version)
    return RhoIncrementalFallbackReason::
        SCALING_VERSION_CHANGED;
  if (parent.inf_version != current.inf_version)
    return RhoIncrementalFallbackReason::INF_VERSION_CHANGED;
  if (parent.canonical_version != current.canonical_version)
    return RhoIncrementalFallbackReason::
        CANONICAL_VERSION_CHANGED;
  if (parent.ordered_columns != current.ordered_columns)
    return RhoIncrementalFallbackReason::
        COLUMN_IDENTITY_CHANGED;
  if (parent.endpoint_conflict_version !=
      current.endpoint_conflict_version)
    return RhoIncrementalFallbackReason::CONFLICT_CHANGED;
  if (parent.service != current.service ||
      parent.urgency != current.urgency ||
      parent.root_delay != current.root_delay)
    return RhoIncrementalFallbackReason::COLUMN_VALUE_CHANGED;
  return RhoIncrementalFallbackReason::NONE;
}
...
{
  switch (reason) {
    case RhoIncrementalFallbackReason::NONE:
      return "NONE";
    case RhoIncrementalFallbackReason::NO_PARENT_STATE:
      return "NO_PARENT_STATE";
    case RhoIncrementalFallbackReason::STALE_OR_REWIRED_PARENT:
      return "STALE_OR_REWIRED_PARENT";
    case RhoIncrementalFallbackReason::SHAPE_CHANGED:
      return "SHAPE_CHANGED";
    case RhoIncrementalFallbackReason::COLUMN_IDENTITY_CHANGED:
      return "COLUMN_IDENTITY_CHANGED";
    case RhoIncrementalFallbackReason::COLUMN_VALUE_CHANGED:
      return "COLUMN_VALUE_CHANGED";
    case RhoIncrementalFallbackReason::MODE_CHANGED:
      return "MODE_CHANGED";
    case RhoIncrementalFallbackReason::CONFLICT_CHANGED:
      return "CONFLICT_CHANGED";
    case RhoIncrementalFallbackReason::OBJECTIVE_VERSION_CHANGED:
      return "OBJECTIVE_VERSION_CHANGED";
    case RhoIncrementalFallbackReason::SCALING_VERSION_CHANGED:
      return "SCALING_VERSION_CHANGED";
    case RhoIncrementalFallbackReason::INF_VERSION_CHANGED:
      return "INF_VERSION_CHANGED";
    case RhoIncrementalFallbackReason::CANONICAL_VERSION_CHANGED:
      return "CANONICAL_VERSION_CHANGED";
    case RhoIncrementalFallbackReason::STATE_VALIDATION_FAILED:
      return "STATE_VALIDATION_FAILED";
    case RhoIncrementalFallbackReason::SHADOW_MISMATCH:
      return "SHADOW_MISMATCH";
  }
  return "INVALID_RHO_FALLBACK_REASON";
}
...
{
  RhoReuseDecision out;
  if (!parent.has_value()) {
    out.fallback =
        RhoIncrementalFallbackReason::NO_PARENT_STATE;
    return out;
  }
  if (!transition_valid || parent_stale) {
    out.fallback = RhoIncrementalFallbackReason::
        STALE_OR_REWIRED_PARENT;
    return out;
  }
  out.fallback = classify_rho_column_model_change(
      parent->column_model, current_model);
  if (out.fallback != RhoIncrementalFallbackReason::NONE)
    return out;
  if (static_cast<int>(current_rows.size()) !=
          parent->optimum.real_rows ||
      static_cast<int>(current_model.ordered_columns.size()) !=
          parent->optimum.columns) {
    out.fallback =
        RhoIncrementalFallbackReason::SHAPE_CHANGED;
    return out;
  }
  for (const auto& row : current_rows)
    if (static_cast<int>(row.eligibility.size()) !=
        parent->optimum.columns) {
      out.fallback =
          RhoIncrementalFallbackReason::SHAPE_CHANGED;
      return out;
    }
  if (!rho_node_state_shape_valid(*parent) ||
      !validate_rho_assignment_state(
          parent->optimum.real_cost, parent->optimum)) {
    out.fallback = RhoIncrementalFallbackReason::
  ...
  out.may_repair = true;
  out.fallback = RhoIncrementalFallbackReason::NONE;
  for (size_t row = 0; row < current_rows.size(); ++row)
    if (current_rows[row] != parent->row_fingerprint[row])
      out.changed_rows.push_back(static_cast<int>(row));
  return out;
}
...
size_t rho_node_assignment_state_payload_bytes(
    const RhoNodeAssignmentState& state)
{
  size_t total = sizeof(RhoNodeAssignmentState);
  const auto add = [&](size_t count, size_t element_size) {
    if (count != 0 &&
        element_size >
            (std::numeric_limits<size_t>::max() - total) /
                count)
      throw std::overflow_error(
          "rho state payload size overflow");
    total += count * element_size;
  };

  add(state.optimum.mate_l.size(), sizeof(int));
  add(state.optimum.mate_r.size(), sizeof(int));
  add(
      state.optimum.row_potential.size(),
      sizeof(RhoWideCost));
  add(
      state.optimum.column_potential.size(),
      sizeof(RhoWideCost));
  add(
      state.optimum.real_cost.size(),
      sizeof(std::vector<RhoCost>));
  for (const auto& row : state.optimum.real_cost)
    add(row.size(), sizeof(RhoCost));

  add(
      state.column_model.ordered_columns.size(),
      sizeof(RhoColumnKey));
  add(state.column_model.service.size(), sizeof(RhoCost));
  add(state.column_model.urgency.size(), sizeof(RhoCost));
  add(state.column_model.root_delay.size(), sizeof(RhoCost));
  add(
      state.column_model.endpoint_conflict_version.size(),
      sizeof(uint64_t));

  add(
      state.row_fingerprint.size(),
      sizeof(RhoRowFingerprint));
  for (const auto& row : state.row_fingerprint)
    add(row.eligibility.size(), sizeof(uint8_t));
  add(
      state.mate.size(),
      sizeof(std::optional<RhoColumnKey>));
  add(
      state.anchor_used.size(),
      sizeof(std::optional<RhoColumnKey>));
  return total;
}
    ...
    const auto accumulate_rho_telemetry =
        [&](const RhoMatchTelemetry& telemetry, bool execute) {
          ...
          stats->rho_incremental_augmentations +=
              telemetry.incremental_augmentations;
          stats->rho_shadow_mismatches +=
              telemetry.shadow_mismatches;
          stats->rho_incremental_copy_time_ms +=
              telemetry.incremental_copy_time_ms;
          stats->rho_incremental_repair_time_ms +=
              telemetry.incremental_repair_time_ms;
          stats->rho_incremental_full_time_ms +=
              telemetry.incremental_full_time_ms;
          stats->rho_incremental_state_bytes_total +=
              telemetry.incremental_state_bytes;
          stats->rho_incremental_state_bytes_max = std::max(
              stats->rho_incremental_state_bytes_max,
              telemetry.incremental_state_bytes);
          if (telemetry.incremental_changed_rows_valid) {
            if (telemetry.incremental_changed_rows == 0)
              ++stats->rho_incremental_changed_rows_0;
            else if (telemetry.incremental_changed_rows == 1)
              ++stats->rho_incremental_changed_rows_1;
            else if (telemetry.incremental_changed_rows == 2)
              ++stats->rho_incremental_changed_rows_2;
            else
              ++stats->rho_incremental_changed_rows_gt2;
          }
          switch (telemetry.incremental_fallback) {
            case RhoIncrementalFallbackReason::NONE:
              break;
            case RhoIncrementalFallbackReason::NO_PARENT_STATE:
              ++stats->rho_fallback_no_parent_state;
              break;
            case RhoIncrementalFallbackReason::
                STALE_OR_REWIRED_PARENT:
