...
namespace
{
                              ...
                              TAPFAssignmentStats* stats)
  {
    ...
    stats->time_ms += std::chrono::duration_cast<std::chrono::nanoseconds>(
                          Time::now() - t_start)
                          .count() /
                      1000000.0;
  }
...
TAPFAssignmentResult assign_tapf_tasks(
    ...
    TAPFAssignmentStats* stats,
    const TAPFAssignmentServiceCostState& service_cost_state,
    const std::vector<int>& motion_state_ids,
    const TAPFDistanceOverride& distance_override)
{
  const auto t_start = Time::now();
  (void)motion_state_ids;
  ...
  auto result =
      HungarianAssignment(build_cost_matrix(ins, D, C, previous_assignment,
                                            sticky_penalty, service_cost_state,
                                            distance_override))
          .solve();
  record_assignment_time(t_start, stats);
  return result;
  ...
  auto motion_cost = [&](const int i, const int j) -> int {
    ...
    const auto partial_remaining =
        i < static_cast<int>(
            ...
            ? std::max(0, service_cost_state.partial_remaining_by_agent[i])
            : 0;
    const auto motion_state = i < static_cast<int>(motion_state_ids.size())
                                  ? motion_state_ids[i]
                                  : -1;
    const auto key =
        row_cache_key(i, C[i], motion_state, partial_task, partial_remaining);
    if (state.row_cost_cache == nullptr) state.reset_row_cost_cache();
    auto iter = state.row_cost_cache->find(key);
    if (iter == state.row_cost_cache->end()) {
