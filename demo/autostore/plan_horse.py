...
# board == the horse's bounding box, exactly (width 3, length 5 or 7)
NR, NC = (3, 5) if MINI else (3, 7)
LEG, BODY, HEAD = "leg", "body", "head"
HEX = {LEG: "#8b5a2b", BODY: "#f1f1eb", HEAD: "#4a4a55"}
if MINI:
    LEG_ZS, BODY_ZS = [0], [1]
    LEG_COLS = [(0, 1), (2, 1), (0, 3), (2, 3)]
    BODY_COLS = [(r, c) for r in range(3) for c in range(1, 4)]
    EXTRA = [((1, 1), 2, HEAD),           # neck
             ((1, 0), 2, HEAD),           # head, cantilevered
             ((1, 4), 1, HEAD)]           # tail, cantilevered
    ROBOT_STARTS = [(0, 0), (2, 2), (0, 4)]
    WORK = HERE / "work_horse_mini"
    OUT = Path("/tmp/plan_horse_mini.js")
else:
    LEG_ZS, BODY_ZS = [0, 1, 2], [3, 4]
    LEG_COLS = [(0, 1), (2, 1), (0, 5), (2, 5)]
    BODY_COLS = [(r, c) for r in range(3) for c in range(1, 6)]
    EXTRA = [((1, 1), 5, HEAD), ((1, 1), 6, HEAD),      # neck
             ((1, 0), 6, HEAD),                          # head
             ((1, 6), 4, HEAD), ((1, 6), 5, HEAD)]       # tail
    ROBOT_STARTS = [(0, 0), (1, 0), (2, 0), (0, 6), (1, 6), (2, 6)]
    WORK = HERE / "work_horse"
    OUT = HERE / "plan_horse.js"
CELLS = [(r, c) for r in range(NR) for c in range(NC)]
TMPL = {cell: {} for cell in CELLS}
for cell in LEG_COLS:
    for z in LEG_ZS:
        TMPL[cell][z] = LEG
for cell in BODY_COLS:
    for z in BODY_ZS:
        TMPL[cell][z] = BODY
for cell, z, color in EXTRA:
    TMPL[cell][z] = color
MAX_Z = max(z for col in TMPL.values() for z in col)
MAX_H = MAX_Z + 2               # parking above the finished structure
RAIL = MAX_H + 2
PILE_CAP = 3 if MINI else 4     # initial random pile height cap
FOOT = CELLS
LEGS = LEG_COLS                 # viz: highlighted tiles
...
STICKY = os.environ.get("DD_DEMO_STICKY", "0") == "1"
...
def main() -> int:
    ...
    for b in bricks:                    # random solid piles on the board
        open_cols = [c for c in FOOT if world.top_z(c) + 1 < PILE_CAP]
    ...
    while not done() or serving:
        ...
        def sim_uncover(color, avoid, budget):
        ...
        def try_build(cell, z, depth, visiting):
            ...
            while any(z2 not in TMPL[cell] and z2 in sim[cell]
                ...
                if not sim_park_top(cell) or guard > MAX_H:
                    return False
                guard += 1
            ...
            if src is None:
                if sim_uncover(color, {cell}, budget=2) is None:
                    return False
                src = sim_find_source(color, {cell})
                if src is None:
                    return False
