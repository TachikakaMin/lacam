...
WORK = HERE / "work_event"

ROWS, COLS = 9, 16
PYR_R0, PYR_C0, PYR_BASE, PYR_LEVELS = 2, 2, 5, 3
MAX_H = 3
RAIL = MAX_H + 2
SRC = [(r, c) for r in range(2, 6) for c in range(10, 13)]
...
def main() -> int:
    ...
    while not template_done() or held:
        ...
        used_src = set()
        for cell in mismatch:           # dig the wrong brick back out
        ...
        res = subprocess.run(
            [str(BINARY), str(ypath), str(TIME_LIMIT), str(ppath),
             "0", "lacam"],
    ...
    plan = {
        ...
        "meta": {"gantry": RAIL, "statsLine":
                 f"事件驱动重规划 · {total} 块 · {replans} 次规划 · 挖掘 {n_dig} 次"},
    }
