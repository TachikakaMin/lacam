...
# high-density rearrangement: board nearly full of bricks; goal = re-sort
# them into color layers (z0 blue, z1 yellow, ...).  A few TRANSPARENT
# staging cells carry no template: they exist purely as parking columns.
NR = NC = 3 if MINI else 5
BLUE, YEL, RED = "blue", "yellow", "red"
HEX = {BLUE: "#3b7dd8", YEL: "#e8c531", RED: "#d84a3b"}
if MINI:
    STAGE = [(0, 0)]
    LAYERS = [BLUE, YEL]
    ROBOT_STARTS = [(0, 2), (1, 0), (2, 1)]
    WORK = HERE / "work_dense_mini"
    OUT = Path("/tmp/plan_dense_mini.js")
else:
    STAGE = [(0, 0), (2, 2), (4, 4)]
    LAYERS = [BLUE, YEL, RED]
    ROBOT_STARTS = [(0, 2), (2, 0), (4, 2), (2, 4), (1, 3), (3, 1)]
    WORK = HERE / "work_dense"
    OUT = HERE / "plan_dense.js"
CELLS = [(r, c) for r in range(NR) for c in range(NC)]
TMPL = {cell: ({} if cell in STAGE else
               {z: color for z, color in enumerate(LAYERS)})
        for cell in CELLS}
MAX_Z = len(LAYERS) - 1
MAX_H = MAX_Z + 2               # one slot of parking headroom everywhere
RAIL = MAX_H + 2
PILE_CAP = MAX_H                # near-full initial density
FOOT = CELLS
LEGS = STAGE                    # viz: transparent staging cells highlighted
