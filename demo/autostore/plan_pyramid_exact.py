...
ROWS, COLS = 5, 5
PYR_R0, PYR_C0, PYR_BASE, PYR_LEVELS = 0, 0, 5, 3
MAX_H = 3
RAIL = MAX_H + 2
...
# grid == final structure footprint: NO external buffer at all;
# temporary parking uses spare capacity on top of site columns
BUF = []
ROBOT_STARTS = [(0, 0), (0, 2), (0, 4), (4, 0), (4, 2), (4, 4)]
...
def main() -> int:
    ...
    while not template_done() or held:
        ...
        used_src = set()
        def want_of(c):
            return [COLOR_BY_LEVEL[z] for z in range(1, tmpl[c] + 1)]
        def pick_park(near, exclude):
            cand = []
            for c in SRC:
                if c in exclude or c in used_goal or c in used_src:
                    continue
                if len(world[c]) >= MAX_H:
                    continue
                w = want_of(c)
                if world[c] == w:
                    rank = 0          # completed column: safe spare slots
                elif world[c] != w[:len(world[c])]:
                    rank = 1          # already mismatched anyway
                else:
                    rank = 2          # correct prefix under construction
                cand.append((rank, abs(c[0]-near[0]) + abs(c[1]-near[1]), c))
        ...
        for rob, info in sorted(held.items()):     # committed bricks first
            ...
            if not goal_still_valid(g, color):     # commitment expired
                ...
                if fresh:
                    ...
                    g = pick_park(robots[rob], set())
                    assert g is not None, "no parking slot left"
    ...
    plan = {
        ...
        "meta": {"gantry": RAIL, "statsLine":
                 f"整板重建(地图=成品底面) · {total} 块 · {replans} 次规划 · 挖掘 {n_dig} 次"},
    }
