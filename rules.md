...
* 任何实验（benchmark、demo 规划、参数扫描等）必须先跑**小实验**验证：缩小规模（更小地图/更少 agent/更少列/更少 seed），并对整个小实验强制限时 **1 分钟以内**（如 `timeout 60`）。
* 小实验必须完整通过（结果正确、无 livelock、无异常慢的段）之后，才允许启动大实验。小实验暴露出的性能异常按 bug 处理，先修再扩大规模。
* 大实验必须**并行跑**：多 seed / 多 case 用多进程并行（工作目录按 seed/case 隔离），并且放到后台执行、定期查看进度，不要前台阻塞等待。
