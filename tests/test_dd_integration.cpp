...
// explicitly).  With RNG off, the rollout trace must equal an independent
// generation that follows the same fixed guidance-refresh protocol.
TEST(dd_integration, rollout_steps_match_fresh_generation)
{
  constexpr size_t GUIDANCE_REFRESH_STEPS = 8;
  DDInstance dd;
  dd.grid = DDGrid({"......", "......", "......"});
  ...
  // Ground truth: each step is generated by a fresh planner from the same
  // state.  Guidance is rebuilt every eight steps and frozen in between,
  // exactly as in the production rollout; scratch state cannot leak.
  std::unique_ptr<CarrierGuidance> frozen_guide;
  ...
  for (size_t t = 0; t < r.ops.size(); ++t) {
    ...
    if (t % GUIDANCE_REFRESH_STEPS == 0 || frozen_guide == nullptr) {
      fresh.attach_carrier_guidance(node.get(), false, frozen_guide.get());
      frozen_guide = std::make_unique<CarrierGuidance>(*node->guide);
      frozen_order = node->order;
    } else {
      node->guide = std::make_unique<CarrierGuidance>(*frozen_guide);
      node->order = frozen_order;
      node->constraint_order = frozen_order;
    }
    TAPFConstraint root;
