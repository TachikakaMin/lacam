// PROTECTED fixed-goal transfer-continuity scope contract.
...
TEST(dd_vacancy_continuity_fixed_goal_scope,
     singleton_goals_do_not_carry_history_into_the_new_epoch)
{
  DDInstance ins;
  ins.grid = DDGrid({"....", "...."});
  ...
  const auto selected =
      carrier_detail::root_first_transfer_continuity(
          guidance.upper_epoch->task_graph);
  EXPECT_NE(selected.find(stable_root), selected.end())
      << "the root must remain active; only historical tie-breaking is scoped out";
  EXPECT_TRUE(
      guidance.upper_epoch->transfer_continuity.empty());
}
