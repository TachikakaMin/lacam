#!/usr/bin/env python3
"""Adaptively run LaCAM-TAPF experiment shards while reserving CPU threads."""
...
def log(path: Path, message: str) -> None:
    line = f"[adaptive-lacam] {time.strftime('%Y-%m-%dT%H:%M:%S%z')} {message}\n"
...
def build_runner_cmd(args: argparse.Namespace, jobs: int) -> list[str]:
    ...
    for path in args.exp1_dir:
        cmd.extend(["--exp1-dir", str(path)])
    for path in args.exp2_dir:
        cmd.extend(["--exp2-dir", str(path)])
    cmd.extend(
        [
            "--skip-ir-suite",
            "--methods",
...
def start_runner(args: argparse.Namespace, jobs: int, log_path: Path) -> int:
    ...
    with run_log.open("a", encoding="utf-8") as f:
        f.write(f"\n[adaptive_lacam_restart] {time.strftime('%Y-%m-%dT%H:%M:%S%z')} jobs={jobs}\n")
...
def main() -> int:
    ...
    parser.add_argument("--methods", nargs="+", default=["lacam_dfs", "lacam_focal_h"])
    ...
    if not args.exp1_dir and not args.exp2_dir:
        raise SystemExit("at least one --exp1-dir or --exp2-dir is required")
