#!/usr/bin/env python3
"""Adaptively run ITA-CBS fixtures while reserving CPU threads."""
...
def log(path: Path, message: str) -> None:
    line = f"[adaptive-itacbs] {time.strftime('%Y-%m-%dT%H:%M:%S%z')} {message}\n"
...
def build_runner_cmd(args: argparse.Namespace, jobs: int) -> list[str]:
    ...
    for path in args.fixture_dir:
        cmd.extend(["--fixture-dir", str(path)])
    cmd.extend(
        [
            "--map-dir",
            str(args.map_dir),
...
def start_runner(args: argparse.Namespace, jobs: int, log_path: Path) -> int:
    ...
    with run_log.open("a", encoding="utf-8") as f:
        f.write(f"\n[adaptive_itacbs_restart] {time.strftime('%Y-%m-%dT%H:%M:%S%z')} jobs={jobs}\n")
...
def main() -> int:
    ...
    parser.add_argument("--itacbs-timeout", type=float, default=10.0)
    ...
    log(
        ...
        (
            ...
            f"time_limit={args.time_limit} itacbs_timeout={args.itacbs_timeout}"
        ),
    )
